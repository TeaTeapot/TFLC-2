...
namespace TextEditor
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            dataGridViewResults.Columns.Add("Code", "Код");
            dataGridViewResults.Columns.Add("Type", "Тип");
            dataGridViewResults.Columns.Add("Value", "Лексема");
            dataGridViewResults.Columns.Add("Position", "Позиция");
        ...
        private void OnStartAnalysis(object sender, EventArgs e)
        {
            ...
            var (tokens, errors) = _scanner.Scan(inputText);
            _lastTokens = tokens;
            dataGridViewResults.Rows.Clear();
            ...
            {
                if (row.Cells["Code"].Value != null && (int)row.Cells["Code"].Value == -1)
                {
            ...
            if (errorCount > 0)
            {
                MessageBox.Show($"Обнаружено ошибок: {errorCount}", "Результат анализа",
        ...
        {
            if (e.RowIndex >= 0 && _lastTokens != null && e.RowIndex < _lastTokens.Count)
            {
                var token = _lastTokens[e.RowIndex];
                int charIndex = 0;
                string[] lines = textBoxEditor.Lines;
                for (int i = 0; i < token.Line - 1; i++)
                {
                    if (i < lines.Length)
                        charIndex += lines[i].Length + Environment.NewLine.Length;
                }
                charIndex += token.StartPos;
                textBoxEditor.Focus();
                textBoxEditor.Select(charIndex, token.Value.Length);
            }
        }
...
- Сохранить как - сохраняет документ с новым именем и выбором пути
...
ПОДДЕРЖКА СКАНЕРА
===========================================
При нажании на ""Пуск"" программа проведет сканирование текста и выведет найденные лексемы на поле вывода.";
